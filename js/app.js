...
(function(){
        ...
        if(http.status === 200 || http.status === 304){
          ...
          //console.log(res)
            if(res.todos){
              ...
              hdateVal = document.createTextNode(value['date'])
              hdate.appendChild(hdateVal);

              htimeVal = document.createTextNode(value.time)
              htime.appendChild(htimeVal);

              pDetailVal= document.createTextNode(value.task)
  ...
  formHandler.addEventListener('submit', function(e){
    ...
    data.date = document.getElementsByClassName('day')[0].value + " " +
